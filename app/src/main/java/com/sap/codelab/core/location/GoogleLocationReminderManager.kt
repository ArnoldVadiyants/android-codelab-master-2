...
import com.sap.codelab.core.receiver.LocationReminderBroadcastReceiver
...
internal class GoogleLocationReminderManager(context: Context) : LocationReminderManager {
    ...
    private val geofencePendingIntent by lazy { LocationReminderBroadcastReceiver.createPendingIntent(appContext) }
    ...
    override suspend fun addReminder(memoId: Long, latitude: Double, longitude: Double) {
        ...
        val request = buildGeofencingRequest(buildGeofence(memoId, latitude, longitude))

        // Add memo to geofences
        addGeofence(request, memoId, latitude, longitude)
    }
        ...
        suspendCancellableCoroutine { continuation ->
            geofencingClient.addGeofences(request, geofencePendingIntent)
                ...
                .addOnFailureListener {
                    Log.e(
                        "LocationReminder",
                        "Failed to add geofence for memoId: $memoId latitude: $latitude longitude: $longitude"
                    )
    ...
    private fun buildGeofence(memoId: Long, latitude: Double, longitude: Double): Geofence =
        Geofence.Builder()
